...
class WordleGame:
  ...
  def update_labels(self):
    ...
    if correct_counter == 5:
      ...
      self.restart()
    if self.attempts >= 6:
      self.display("You ran out of attempts, You lose! \n \tThe word was " + self.current_word)
      self.entry.config(state= tk.DISABLED)
      self.submit.config(state= tk.DISABLED)
      self.restart()
  ...
  def restart(self):
    global Keyboard_Dictionary
    again = messagebox.askyesno("Restart?","Do you want to play again?")
    if again == True:
      print("restart")
      Keyboard_Dictionary = {}
      newGame = WordleGame(window)
      newGame.keyboard()
      newGame.make_keyboard()#clears keyboard
